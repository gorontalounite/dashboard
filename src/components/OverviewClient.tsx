  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          { label: "Total Reports", val: totalReports, icon: "📋", color: "from-orange-500/20 to-pink-500/20" },
          { label: "Total Views", val: totalViews, icon: "👁️", color: "from-blue-500/20 to-cyan-500/20" },
          { label: "Total Interaksi", val: totalInteractions, icon: "💬", color: "from-purple-500/20 to-pink-500/20" },
          { label: "Accounts Reached", val: totalReached, icon: "📡", color: "from-emerald-500/20 to-teal-500/20" },
        ].map((item) => (
          <div key={item.label} className={`glass rounded-2xl p-5 bg-gradient-to-br ${item.color}`}>
              ...
              >
                ...
                <div className="text-right flex-shrink-0">
                  <p className="text-white font-display font-bold text-sm">
                    {formatNumber(report.metrics?.views ?? 0)}
                  </p>
